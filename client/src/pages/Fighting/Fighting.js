...
const enemies = [
  {
    ...
    pathName:'BossBattle',
    link: 'Battle Boss',
    icon: <SparklesIcon className=" ml-2 justify-center h-5 w-5 text-gray-500" aria-hidden="true" />,
    imageUrl:
    ...
    },
    {
      ...
      icon: <ChipIcon className=" ml-2 justify-center h-5 w-5 text-gray-500" aria-hidden="true" />,
      imageUrl:
      ...
      <div className="mx-auto py-12 px-4 max-w-7xl sm:px-6 lg:px-8 lg:py-24">
        <div className="space-y-12">
          <div className="space-y-5 sm:space-y-4 md:max-w-xl lg:max-w-3xl xl:max-w-none">
            <h2 className="text-3xl font-extrabold text-white tracking-tight sm:text-4xl">Prove Your Worth</h2>
          ...
          <ul role="list" className="space-y-4 sm:grid sm:grid-cols-2 sm:gap-6 sm:space-y-0 lg:grid-cols-3 lg:gap-8">
            {enemies.map((enemy) => (
              <li key={enemy.name} className="py-10 px-6 bg-gray-800 text-center rounded-lg xl:px-10 xl:text-left">
                <div className="space-y-6 xl:space-y-10">
                  <img className="mx-auto h-40 w-40 rounded border-4 border-black xl:w-56 xl:h-56" src={enemy.imageUrl} alt="" />
                  <div className="space-y-2 xl:flex xl:items-center xl:justify-between">
                    <div className="font-medium text-lg leading-6 space-y-1">
                      <h3 className="text-white">{enemy.name}</h3>
                      <p className="text-indigo-400 flex justify-center">Rewards 1 {enemy.icon} </p>
